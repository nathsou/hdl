...
export type SExpr = {
  name: string,
  attributes: (StringToken | NumberToken | SymbolToken)[],
  children: SExpr[],
};
...
export const showSExpr = ({ name, attributes, children }: SExpr, identation = 0): string => {
  if (children.length == 0) {
    return `(${name} ${attributes.map(showToken).join(' ')})`;
  }

  const ident = '  '.repeat(identation);
  return `${ident}(${name} ${attributes.map(showToken).join(' ')}\n${ident}  ${children.map(e => showSExpr(e, identation + 1)).join(`\n${ident}  `)}\n${ident})`;
};
...
const parsePin = (expr: SExpr): SymbolPin => {
  ...
  return {
    name: childrenByName.get('name')!.attributes[0].value.toString().toLowerCase(),
    number: Number(childrenByName.get('number')!.attributes[0].value),
  };
};
...
const parseRectangle = (rect: SExpr): BoundingRect => {
  ...
  const [x1, y1] = children.get('start')!.attributes.map(n => Number(n.value));
  const [x2, y2] = children.get('end')!.attributes.map(n => Number(n.value));
